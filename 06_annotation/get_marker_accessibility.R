###################################################################################################
## get marker accessibility scores 
###################################################################################################

# load arguments
args <- commandArgs(trailingOnly=T)
if(length(args) != 7){stop("Rscript plot_marker_accessibility.R [meta] [gene_activity] [pcs.txt] [markers.bed] [threads] [prefix] [dirout]")}
...
saveRDS(out, file = file.path(dirout, paste0(prefix,"_marker_clusters.rds")))
saveRDS(out$activity, file = file.path(dirout, "All_clusters_Gene_activity.rds"))
saveRDS(out$impute.activity, file = file.path(dirout, "All_clusters_Gene_activity.imputed.rds"))
