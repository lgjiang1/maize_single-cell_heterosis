...
BASE="3_remove_false_ACR_calls/filtered_peaks_in_hybrids/B73"   # Replace B73 (Ki3 or Oh43) with target genotype if needed
PEAK_DIR=${BASE}
TN5_DIR=${BASE}
...
PREFIX=$(printf '%s\n' "${PEAK_DIR}"/*.narrowPeak | sed -n "${SLURM_ARRAY_TASK_ID}{s#.*/##; s/\.narrowPeak$//; p}")
echo "[INFO] Processing ${PREFIX}"
mkdir -p "${WORK}/${PREFIX}"
RAW_PEAK=${PEAK_DIR}/${PREFIX}.narrowPeak 
...
awk 'BEGIN{OFS="\t"}{print $1,$2,$3,$4,$9,$10,$11}' "${RAW_PEAK}" > "${WORK}/${PREFIX}/${PREFIX}.peaks7.bed"
sort -k1,1 -k2,2n "${WORK}/${PREFIX}/${PREFIX}.peaks7.bed" > "${WORK}/${PREFIX}/${PREFIX}.peaks.sorted.bed"
