...
if(length(args) != 2){
  stop("Usage: Rscript filter_low_qual_cells.R <input.rds> <output_prefix>")
}
...
# ----------- Initial Filtering -----------
a$meta$qc_check <- ifelse(
  a$meta$total >= 1000 &
  a$meta$pTSS > 0.2 &
  a$meta$FRiP > 0.2 &
  a$meta$pOrg < 0.05, 1, 0
)
message(" - initial good cells (hard filter): ", sum(a$meta$qc_check == 1))

# ----------- z-score Filtering -----------
ref_idx <- which(a$meta$qc_check == 1)
a$meta$tss_z <- (a$meta$pTSS - mean(a$meta$pTSS[ref_idx], na.rm = TRUE)) / sd(a$meta$pTSS[ref_idx], na.rm = TRUE)
a$meta$acr_z <- (a$meta$FRiP - mean(a$meta$FRiP[ref_idx], na.rm = TRUE)) / sd(a$meta$FRiP[ref_idx], na.rm = TRUE)
a$meta$sites_z <- (log10(a$meta$nSites) - mean(log10(a$meta$nSites[ref_idx]), na.rm = TRUE)) / sd(log10(a$meta$nSites[ref_idx]), na.rm = TRUE)
...
a$meta$qc_check <- ifelse(
  a$meta$tss_z < -2 | a$meta$pTSS < 0.2, 0,
  ifelse(a$meta$acr_z < -2 | a$meta$FRiP < 0.2, 0,
    ifelse(a$meta$sites_z < -2, 0,
      ifelse(a$meta$pOrg > 0.05, 0, a$meta$qc_check)
    )
  )
)
message(" - good cells after z-score filtering: ", sum(a$meta$qc_check == 1))

# ------Parse and filter good/bad cells for reference construction --------
...
nonrefs <- a$meta[!rownames(a$meta) %in% rownames(all.refs), , drop = FALSE]

if (nrow(nonrefs) > 0) {
  nonrefs$bREF <- NA_real_
  nonrefs$gREF <- NA_real_
  nonrefs$call <- NA_integer_
  test <- rbind(all.refs, nonrefs)
} else {
  test <- all.refs
}
...
test$call <- ifelse(test$dif > 0 & test$dif > cut.off & test$qc_check == 1, 1, 0)
test$dif <- NULL
